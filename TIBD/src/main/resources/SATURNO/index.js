...
if (pagina == "tarefas") {
    ...
    let user=JSON.parse(sessionStorage.getItem("usuarioCorrente")).id;

    for (i = 0; i < objDados.tarefas.length; i++) {


        if(objDados.tarefas[i].autor===user)
        {
...
let conta = JSON.parse(sessionStorage.getItem("usuarioCorrente")).tarefasConcluidas;
...
function limpaLs(mouse) {
    ...
    //Código do grafico
    let aConcluir = JSON.parse(localStorage.getItem("db")).tarefas.filter(tarefa=>tarefa.autor===JSON.parse(sessionStorage.getItem("usuarioCorrente")).id).length
    aConcluir--
...
if (pagina == "tarefas") {
    for (i = 0; i < objDados2.tarefas.length; i++) {
        ...
        let index = usuarios.usuarios.map((usuario) => {
            ...
            usuarioCorrente.tarefasConcluidas = conta;
            usuarios.usuarios[index].tarefasConcluidas = conta;
